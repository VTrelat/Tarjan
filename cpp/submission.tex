...
\subsection{Pre-conditions imply pre-conditions}
\label{sec:pre-pre}
...
For example, the following lemma asserts that the pre-condition of \prog{dfss} holds at the call from the body of \prog{dfs}. Although some of the conditions of the invariant for the augmented invariant require a few lines of proof, the overall proof is straightforward.
...
The following lemma asserts the pre-condition of the recursive call to \prog{dfs} from \prog{dfss} for a successor that has not been visited yet. Given that the environment of the call is unchanged, the proof of the lemma is entirely automatic.
...
Since the two functions are mutually recursive, the two statements must be proved by simultaneous computational induction, based on the corresponding proof rule generated by Isabelle. The proofs of these implications require substantially more interactions than the proofs of the lemmas of Sect.~\ref{sec:pre-pre}.
...
\textbf{Perhaps explain a complicated reasoning step that makes use of lemma avoiding-explored?}
