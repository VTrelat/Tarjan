...
\begin{frame}
  \begin{isabelle}[Adding a second lemma]
    \isa{
      \lm{} concat\_empty [simp]: "concat \blue{xs} Empty = \blue{xs}"\\
      ~~~\apply{} (induction \blue{xs})\\
      ~~~\apply{} auto
    }
  \end{isabelle}
...
\begin{frame}
  ...
  \begin{isabelle}[Maximal SCC]
    \isa{
      ...
      ~$\wedge$ ($\forall$ \green{S\textquotesingle}. \green{S}$\subseteq$S\textquotesingle $\wedge$ is\_subscc \green{S\textquotesingle} $\longrightarrow$ \green{S\textquotesingle} = \green{S})"
    }
  \end{isabelle}
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item Parallel algorithm for computing SCC ? Proof ?
  \end{itemize}
\end{frame}
