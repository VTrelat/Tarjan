...
function unite :: "'v \<Rightarrow> 'v \<Rightarrow> 'v env" where
  "unite v w e =
      (if (S e v = S e w) then e
               ...
               r'= hd(tl(stack e));
               e'= e \<lparr> stack := tl(stack e), S := S e (r \<leftarrow> S e r \<union> S e r', r' \<leftarrow> S e r \<union> S e r')\<rparr>
          in unite v w e')"

function dfs :: "'v \<Rightarrow> 'v env \<Rightarrow> 'v env" and dfss :: "'v \<Rightarrow> 'v set \<Rightarrow> 'v env \<Rightarrow> 'v env" where
  "dfs v e =
  (let e1 = e1\<lparr>visited := visited e \<union> {v}, stack := (v # stack e)\<rparr>;
       e' = dfss (succ v) e1
  in if v = hd(stack e')
      then e'\<lparr>sccs:=sccs e' \<union> (S e v), explored:=explored e' \<union> (S e u), stack:=tl(stack e')\<rparr>
    else e')"
| "dfss v vs e =
   (if vs = {} then e
